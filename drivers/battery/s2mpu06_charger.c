...
#define DEFAULT_CHARGING_CURRENT 700
...
#if EN_VINVR_IRQ
#define MIN_INPUT_CURRENT	400
#define REDUCE_CURRENT_STEP	40
...
static bool s2mpu06_chg_init(struct s2mpu06_charger_data *charger)
{
	...
	s2mpu06_update_reg(charger->client, 0x3C, 0x40, 0x40);
...
static int s2mpu06_get_charging_health(struct s2mpu06_charger_data *charger)
{
	...
	/* if charger IC reset, re-call init function */
    s2mpu06_read_reg(charger->client, S2MPU06_CHG_REG_CTRL4, &val);	
	if ((val & 0x01) == 1) {
		s2mpu06_chg_init(charger);
		pr_info("%s, Charger IC reset! call initial function\n", __func__);
	}
	/* clear watchdog */
	s2mpu06_update_reg(charger->client, S2MPU06_CHG_REG_CTRL12,
			   0x01, 0x03);
